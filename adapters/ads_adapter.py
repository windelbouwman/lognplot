...
def parse_ads_entries(entries_data: bytes, size: int, count: int):
    ...
    while total_bytes < size:
        ...
        parsed_entries.append(
            Entry(name, typ, comment, entry_datatype, entry_datatype_size)
        )
...
def main():
    ...
    def callback_lreal(handle, name, timestamp, value):
        client.send_sample(
            name,
            time.mktime(timestamp.timetuple()) * 1e3 + timestamp.microsecond / 1e3,
            value,
        )
    ...
    upload_info = plc.read(
        pyads.constants.ADSIGRP_SYM_UPLOADINFO, 0, pyads.structs.SAdsSymbolUploadInfo
    )
    print(
        "nSymSize: "
        + str(upload_info.nSymSize)
        + "\nnSymbols: "
        + str(upload_info.nSymbols)
    )
    ...
    parsed_entries = parse_ads_entries(
